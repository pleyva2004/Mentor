...
export function CursorPrompt({ className }: CursorPromptProps) {
  ...
  // Get current section from resume store
  const { getCurrentSection } = useResumeStore();
    ...
    try {
      ...
      const requestBody = {
        ...
        context: {
          ...
          has_resume_data: !!resumeData.header,
          sections_confirmed: {
            header: resumeData.header?.isConfirmed || false,
            projects: resumeData.projects?.isConfirmed || false,
            skills: resumeData.skills?.isConfirmed || false,
            experience: resumeData.experience?.isConfirmed || false,
            education: resumeData.education?.isConfirmed || false,
          }
        }
      };
  ...
  return (
    <>
      ...
      <button
        onClick={toggleOpen}
        className={`fixed bottom-6 right-6 bg-blue-600 text-white rounded-full p-4 shadow-lg hover:bg-blue-700 transition-colors z-50 ${className}`}
      ...
      {isOpen && (
        <div className="fixed bottom-24 right-6 w-96 bg-white rounded-lg shadow-2xl border border-gray-200 z-50">
          <div className="flex items-center justify-between p-4 border-b">
            <h3 className="text-lg font-semibold text-gray-800">Resume AI Assistant</h3>
            <button
          ...
          <div className="p-4 max-h-96 overflow-y-auto">
            ...
            {currentSection && (
              <div className="mb-3 text-xs text-gray-500">
                Currently editing: <span className="font-medium capitalize">{currentSection}</span> section
              </div>
            )}
